...
[System.Serializable]
public class InRunAnim : MonoBehaviour, PointAnimI
{
    [Header("Settings")]
    ...
    private void Start()
    {
        avgFactor = 0;
        frame = 0;
        mouseXCtrl = jumpInfo.controlValue * jumperInfo.ControlFactorRun * jumperInfo.MouseXSens;
    }
    ...
    private void Update()
    {
        ...
        if (Random.Range(0f, 1f) < jumpInfo.swingFrequencyOther)
        {
            speed += Random.Range(0.08f, 0.2f)*jumperInfo.SwingsFactorRun * Mathf.Sign(Random.Range(-1, 1));
        }

        axis = Mathf.Sign(axis) * Mathf.Min(5, Mathf.Abs(axis));
        speed = Mathf.Sign(speed) * Mathf.Min(1f, Mathf.Abs(speed));

        Factor = Mathf.Max(1 - Mathf.Abs(axis) / 5, 0.35f) * jumperInfo.lubricationFactor;
